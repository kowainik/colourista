...
import Colourista (ColourMode (..), HasColourMode, formatWith, italic, red, reset, yellow)
...
colouristaSpec = describe "Colourista tests" $ do
    ...
    describe "Colour disabling works" $ do
        ...
        it "Disable formatWith" $
            let ?colourMode = DisableColour
            in formatWith @Text [red, italic] "plain" `shouldBe` "plain"
        it "Disable short bold" $
            let ?colourMode = DisableColour
            in b @Text "thin" `shouldBe` "thin"
        it "DisableColour propagates to custom functions" $
            let ?colourMode = DisableColour
            in customPainting `shouldBe` "not formatted"
        it "DisableColour doesn't propagate to custom functions without constraint" $
            let ?colourMode = DisableColour
            in colourWithIgnore `shouldBe` "\ESC[93m\ESC[3mpretty\ESC[0m"

customPainting :: HasColourMode => Text
customPainting = formatWith [yellow, italic] "not formatted"

colourWithIgnore :: Text
colourWithIgnore = formatWith [yellow, italic] "pretty"
